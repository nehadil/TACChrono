            ...
            if month is not None and mod is None:
                ...
                if day is not None and my_month == dct.month:
                    ...
                    if chrono_list[day].get_value() <= dct.day:
                        chrono_list.append(
                            chrono.ChronoLastOperator(entityID=str(chrono_id) + "entity", start_span=mStart,
                                                      end_span=mEnd, repeating_interval=chrono_list[month].get_id()))
                        chrono_id = chrono_id + 1
                    elif chrono_list[day].get_value() > dct.day:
                        chrono_list.append(
                            chrono.ChronoNextOperator(entityID=str(chrono_id) + "entity", start_span=mStart,
                                                      end_span=mEnd, repeating_interval=chrono_list[month].get_id()))
                        chrono_id = chrono_id + 1

                elif my_month < dct.month:
                    chrono_list.append(
                        chrono.ChronoLastOperator(entityID=str(chrono_id) + "entity", start_span=mStart, end_span=mEnd,
                                                  repeating_interval=chrono_list[month].get_id()))
                    chrono_id = chrono_id + 1

                elif my_month > dct.month:
                    chrono_list.append(
                        chrono.ChronoNextOperator(entityID=str(chrono_id) + "entity", start_span=mStart, end_span=mEnd,
                                                  repeating_interval=chrono_list[month].get_id()))
                    chrono_id = chrono_id + 1
        ...
        if bMonth and not flags["month"]:
            ...
            if (int(textMonth) <= 12):
                chrono_month_entity = chrono.chronoMonthOfYearEntity(entityID=str(chrono_id) + "entity",
                                                                     ...
                                                                     month_type=calendar.month_name[int(textMonth)])
                chrono_id = chrono_id + 1
                chrono_year_entity.set_sub_interval(chrono_month_entity.get_id())
...
def buildSeasonOfYear(s, chrono_id, chrono_list, ref_list):
    ...
    if boo:
        ...
        my_entity = chrono.ChronoSeasonOfYearEntity(entityID=str(chrono_id) + "entity", start_span=abs_Sspan,
                                                    end_span=abs_Espan, season_type=val)
        chrono_id = chrono_id + 1
...
def buildTextYear(s, chrono_id, chrono_list):
    ...
    if boo:
        ...
        my_year_entity = chrono.ChronoYearEntity(entityID=str(chrono_id) + "entity", start_span=abs_Sspan,
                                                 end_span=abs_Espan, value=val)
        chrono_id = chrono_id + 1
...
def buildTextMonthAndDay(s, chrono_id, chrono_list, dct=None, ref_list=None):
    ...
    if boo:
        ...
        abs_Sspan = ref_Sspan + idxstart
        abs_Espan = ref_Sspan + idxend
        my_month_entity = chrono.chronoMonthOfYearEntity(entityID=str(chrono_id) + "entity", start_span=abs_Sspan,
                                                         end_span=abs_Espan, month_type=val)
        chrono_id = chrono_id + 1
        ...
        if (idxend < len(s.getText())):
            ...
            if num is not None:
                if num <= 31:
                    ...
                    my_day_entity = chrono.ChronoDayOfMonthEntity(entityID=str(chrono_id) + "entity",
                                                                  start_span=abs_Sspan, end_span=abs_Espan, value=num)
                    ...
                    if False:  # dct is not None:
                        ...
                        this_dct = datetime.datetime(int(dct.year),
                                                     int(utils.getMonthNumber(my_month_entity.get_month_type())),
                                                     int(my_day_entity.get_value()), 0, 0)
                        if this_dct > dct:
                            chrono_list.append(
                                chrono.ChronoNextOperator(entityID=str(chrono_id) + "entity", start_span=mStart,
                                                          end_span=mEnd, repeating_interval=my_month_entity.get_id()))
                            chrono_id = chrono_id + 1
                        elif this_dct < dct:
                            chrono_list.append(
                                chrono.ChronoLastOperator(entityID=str(chrono_id) + "entity", start_span=mStart,
                ...
                for i in range(0, len(tokenized_text)):
                    ...
                    if num is not None:
                        if num <= 31:
                            ...
                            abs_Espan = ref_Sspan + day_endidx
                            my_day_entity = chrono.ChronoDayOfMonthEntity(entityID=str(chrono_id) + "entity",
                                                                          start_span=abs_Sspan, end_span=abs_Espan,
                                                                          value=num)
                            chrono_list.append(my_day_entity)
                            chrono_id = chrono_id + 1
                            ...
                            if False:  # dct is not None:
                                ...
                                this_dct = datetime.datetime(int(dct.year), int(
                                    utils.getMonthNumber(my_month_entity.get_month_type())),
                                                             int(my_day_entity.get_value()), 0, 0)
                                if this_dct > dct:
                                    chrono_list.append(
                                        chrono.ChronoNextOperator(entityID=str(chrono_id) + "entity", start_span=mStart,
                            ...
                            abs_Sspan = ref_Sspan + year_startidx
                            abs_Espan = ref_Sspan + year_endidx
                            my_year_entity = chrono.ChronoYearEntity(entityID=str(chrono_id) + "entity",
                                                                     start_span=abs_Sspan, end_span=abs_Espan,
                                                                     value=num)
                            chrono_list.append(my_year_entity)
                            my_year_entity.set_sub_interval(my_month_entity.get_id())
        ...
        if (idxstart > 0):
            ...
            if (hasMod):
                if mod_type == "This":
                    chrono_list.append(
                        chrono.ChronoThisOperator(entityID=str(chrono_id) + "entity", start_span=ref_Sspan + mod_start,
                ...
                if mod_type == "Next":
                    chrono_list.append(
                        chrono.ChronoNextOperator(entityID=str(chrono_id) + "entity", start_span=ref_Sspan + mod_start,
                                                  end_span=ref_Sspan + mod_end,
                                                  repeating_interval=my_month_entity.get_id()))
                    chrono_id = chrono_id + 1
                ...
                if mod_type == "Last":
                    # print("FOUND LAST")
                    chrono_list.append(
                        chrono.ChronoLastOperator(entityID=str(chrono_id) + "entity", start_span=ref_Sspan + mod_start,
                                                  end_span=ref_Sspan + mod_end,
                                                  repeating_interval=my_month_entity.get_id(),
    ...
    #     print("**************Yesterday!*****************==============================================")
        ...
        if (my_class == 1):
            my_entity = chrono.ChronoDoseDurationEntity(entityID=str(chrono_id) + "entity", start_span=abs_Sspan,
                                                        end_span=abs_Espan, period_type=getPeriodValue(val), number=None)
            chrono_id = chrono_id + 1
        ...
        if (boo2):
            ...
            if (my_class == 1):
                ...
                my_entity = chrono.ChronoCalendarIntervalEntity(entityID=str(chrono_id) + "entity",
                                                                start_span=abs_Sspan, end_span=abs_Espan,
                                                                calendar_type=val)
                chrono_id = chrono_id + 1
            ...
            # else search for a text number
                ...
                if texNumVal is not None:
                    ...
                    if m is not None:
                        ...
                        # create the number entity
                        my_number_entity = chrono.ChronoNumber(entityID=str(chrono_id) + "entity", start_span=abs_Sspan,
                                                               end_span=abs_Espan, value=texNumVal)
                        chrono_id = chrono_id + 1
                        # append to list
                        chrono_list.append(my_number_entity)
                        # link to interval entity
...
def buildPartOfDay(s, chrono_id, chrono_list):
    ...
    if boo:
        ...
        abs_Sspan = ref_Sspan + idxstart
        abs_Espan = ref_Sspan + idxend
        my_entity = chrono.ChronoPartOfDayEntity(entityID=str(chrono_id) + "entity", start_span=abs_Sspan,
                                                 end_span=abs_Espan, part_of_day_type=val)
        chrono_list.append(my_entity)
        chrono_id = chrono_id + 1
        # check here to see if it has a modifier
...
def buildPartOfWeek(s, chrono_id, chrono_list):
    ...
    if boo:
        ...
        my_entity = chrono.ChronoPartOfWeekEntity(entityID=str(chrono_id) + "entity", start_span=abs_Sspan,
                                                  end_span=abs_Espan, part_of_week_type=val)
        chrono_list.append(my_entity)
        chrono_id = chrono_id + 1
        # check here to see if it has a modifier
...
def build24HourTime(s, chrono_id, chrono_list, flags):
    ...
    if boo and not flags["loneDigitYear"]:
        ...
        ## build minute entity
        min_entity = chrono.ChronoMinuteOfHourEntity(entityID=str(chrono_id) + "entity",
                                                     start_span=ref_Sspan + idxstart + 2,
                                                     end_span=ref_Sspan + idxstart + 4, value=minute)
        ...
        # else:
        hour_entity = chrono.ChronoHourOfDayEntity(entityID=str(chrono_id) + "entity", start_span=ref_Sspan + idxstart,
                                                   end_span=ref_Sspan + idxstart + 2, value=hour)
    ...
    if boo:
        ...
        if val is not None:
            if val == "nearly" or val == "almost":
                my_modifier_entity = chrono.ChronoModifier(str(chrono_id) + "entity", start_span=abs_Sspan,
                                                           end_span=abs_Espan, modifier="Less-Than")
...
def hasTimeZone(tpentity):
    ...
    text_norm = text.translate(str.maketrans(string.punctuation, ' ' * len(string.punctuation)))
    tz = re.search(
        '(AST|EST|EDT|CST|CDT|MST|MDT|PST|PDT|HST|SST|SDT|GMT|UTC|BST|CET|IST|MSD|MSK|AKST|HAST|HADT|CHST|CEST|EEST)',
        text_norm)
...
def hasCalendarInterval(tpentity):
    ...
    terms = ["decades", "decade", "yesterday", "day", "week", "month", "year", "daily", "weekly", "monthly", "yearly",
             "century", "minute", "second", "hour", "hourly", "days", "weeks", "months", "years", "centuries",
...
def hasEmbeddedPeriodInterval(tpentity):
    ...
    terms = ["decades", "decade", "today", "yesterday", "day", "week", "month", "year", "daily", "weekly", "monthly",
             "yearly", "century", "minute", "second", "hour", "hourly", "days", "weeks", "months", "years", "centuries",
             "minutes", "seconds", "hours"]
...
def hasPartOfDay(tpentity):
    ...
    partofday = ["morning", "evening", "afternoon", "night", "dawn", "dusk", "tonight", "overnight", "nights",
                 "mornings", "evening", "afternoons", "noon"]
